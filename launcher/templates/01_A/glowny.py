...
from funkcje_specyficzne_dostawca import *
import konfiguracja
from local_python_transformations.wspolne.stale import kolumnyDoDodania
...
df = dodaj_standardowe_kolumny(df, kolumnyDoDodania)
df = dodaj_miesiac(df, miesiac, rok)
df = dodaj_dystrybutor(df, konfiguracja.dystrybutor)
df = wyczysc_NIP(df, 'NIP')

df = wylicz_standardowe_kolumny(df, df_asortyment, "Nazwa produktu", df_klient, 'ID_Weryf.')
